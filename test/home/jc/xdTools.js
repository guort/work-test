...
YAO:"//yaotv.tvm.cn"
},PAGE={yid:'wx33dc1a5264b4e846',cid:1782,pt:'354e6b14b65b79ad',share:{title:'BTV生活分享有礼，特别的爱给特别的你！',ico:'http://q.cdn.mtq.tvm.cn/open/data/upload/201603/b91b2d52bee7c231b67a99b104ffcc61.jpg',link:'https://wximg.gtimg.com/shake_tv/shaketv_new/zip/8sr40iluno08d4989xb8b4/fx.html',desc:'BTV生活最任性，特别的爱给特别的你！'}};
...
function toSQ($opt){
	...
	if(ls=localStorage.getItem("US"+openid)){
		ls=toObject(ls);
		userInfo.sex=ls.sex;
	...
	userInfo.cn=cn;
		if(lotteryId){
			...
			PAGE.lotteryId=lotteryId;
			if(ls)
			$opt.fun()
			else
			getUSER(openid,"openid="+ttdsbopenid,userInfo,$opt.fun);	
			//check();	
		}else if(openid){		
			userInfo.ttdsbopenid=openid;
			userInfo.ttdsbsig=search.sig;
			getUSER(openid,"openid="+openid,userInfo,$opt.fun)
		}else if(userInfo.isWX){
				url=location.href
				url+=(url.indexOf("?")>1?"&":"?")+"toSQ=en";
				location.replace(HOST.AD+"/oauth?wx_token=46497107fa23&redirecturl="+encodeURIComponent(url))
			}
		//检查绑定
function check(){
	var user=userInfo,a=setAjax('get',HOST.RTS+"/check/check?yyyappid="+PAGE.yid+"&yopenid="+user.openid+"&sigExpire="+user.sigExpire+"&yaoSig="+user.sig+"&"+nocache());
		a.callBack=function($data){
			var data=toObject($data);
			if(data.subscribe)user.guanzhu=1
		};
		a.send();
		}					
	}
...
function XD_YZM($opt){
	...
	function jy(){
		if(window.initGeetest){
			...
			c();r();
			function c(){
				var a=setAjax("GET","//pmall.yaotv.meitiqiao.com/open/geetest/register")
					a.callBack=function($data){
